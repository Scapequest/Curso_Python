...
padrao_codigo = re.compile(r'\d{4}') #\d -> número 0000
padrao_pagamento = re.compile(r'\d{2}/\d{2}/\d{4}') # 00/00/0000
padrao_valor = re.compile(r'\d{1,3}(?:\.\d{3})*,\d{2}') # 000,00
...
def extracao_dados(arquivo, excel):
    ...
    with pdfplumber.open(arquivo) as pdf:
        for pagina in tqdm(pdf.pages, desc='Extraindo dados do arquivo', unit='Página', ncols=100):
    ...
    df = pd.DataFrame(dados_brutos, columns = ['DATA REFERÊNCIA', 'CÓDIGO', 'NOME', 'DATA PAGAMENTO', 'REF', 'VALOR'])
...
def main():
    caminho_arquivo = r'holerite.pdf'
    caminho_excel = caminho_arquivo.replace('.pdf', ".xlsx") #troca o .pdf por .xlsx, usando o mesmo caminho do pdf para criar a planilha.
