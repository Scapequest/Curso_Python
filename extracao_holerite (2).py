...
padrao_codigo = re.compile(r'\d{4}') #\d para numeros \w para letras
padrao_pagamento = re.compile(r'\d{2}/\d{2}/\d{4}')
padrao_valor = re.compile(r'\d{1,3}(?:\.\d{3})*,\d{2}') ##### PERGUNTAR SOBRE ##############
...
def extracao_dados(arquivo, excel):
    ...
    with pdfplumber.open(arquivo) as pdf:
        for pagina in tqdm(pdf.pages, desc= 'Extraindo dados do arquivo', unit='Pagina', ncols=100):
            texto = pagina.extract_text() #funcao do pdfplumber
            linhas = texto.split('\n')
            for linha in linhas:
                if linha.startswith('Mês'):
                    ...
                    ano = int(match_ano.group().replace('.',''))
...
def main_holerite():
    caminho_arquivo = r'C:\Users\aluno\Desktop\minicurso\holerite.pdf'
